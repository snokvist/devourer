...
- `StationSm.h` — the association state machine. **Built 2026-09-21, and the
  shape below was NOT what shipped** — recorded rather than quietly amended:
  - *Written:* a pure transition function over `(event, now_ms)` returning
    actions, with timers and retry budgets as values and the caller sleeping,
    like `src/sensing/DwellExecutor`.
  - *Built:* a stateful class with an internal bounded transmit queue and
    timeouts as `static constexpr`. The clock is still an argument, which is
    the property that mattered — every timeout is assertable headlessly — but
    `tick()` returns no next deadline, so a caller polls rather than sleeps.
    That is worse for a power-managed integrator and better for the harness;
    if a caller ever needs to sleep, `tick()` growing a return value is the
    change, not a rewrite.
  - *States:* `Idle → Authenticating → Associating → FourWay → Connected →
    Failed`. `Scanning` is deliberately absent (scanning needs a radio, so it
    is the harness's); `Deauthed` and `BeaconLost` are `Failure` reasons on
    one `Failed` state rather than states of their own; `Reconnecting` does
    not exist — re-joining is the caller's policy, and that is carried into
    Phase 4 explicitly.
  - `.h/.cpp` became header-only, matching `Ccmp.h`/`Dot11.h`/`StationTable.h`.
- `Eapol.h`, `Supplicant.h` (not `Wpa2Supplicant.h/.cpp`; header-only, for the
  same reason) — the supplicant half of the 4-way.
  **MIC verified before any state change. Replay gate `<=`. Constant-time MIC
  ...
  1, 2, and the minor list.) All five hold as of `03d2478`; zeroize-on-teardown
  was the one that did not until a continuity review noticed the AP half had
  been doing it since Phase 2b and the station half had not.

  **A sixth rule the review added, which is not in PR #335's list because
  #335 did not get far enough to hit it:** the replay counter advances only
  where a MIC has verified. Message 1 carries none, and letting it advance the
  counter meant one forged frame refused every genuine EAPOL-Key for the rest
  of the association — with the station still reporting Connected and keyed.
  See the Phase 3 section of `docs/station-mode-plan.md`.
...
**Phase 3 — pure station logic, offline. DONE 2026-09-21.**
`BssTable`, `StationSm`, `Eapol`, `Supplicant`, over `CryptoOps`. Headless
ctest, plus explicit negative cells for the two PR #335 defects — **a forged
MIC must be rejected**, and **an equal-counter replayed group rekey must be
rejected**. No hardware.

**"A recorded authenticator transcript" was substituted, and here is what
for.** The 4-way runs against an in-process fixture authenticator, which is
weaker than a transcript in one specific way — the fixture shares an author
with the code it tests, so a shared misreading is invisible to it. Two things
cover that gap instead, and between them they are stronger than a transcript
would have been:

- `tests/eapol_kernel_vectors.h` — the four EAPOL-Key frames **hostapd and
  wpa_supplicant actually exchanged**, which is a recorded transcript in
  everything but name, plus the PTK wpa_supplicant derived and the GTK it
  installed. Regenerated by `tests/eapol_capture_vectors.sh` from two virtual
  radios; no hardware.
- `test_library_station_associates()` — this supplicant driven against the
  Phase 2b authenticator, which hand-rolls every EAPOL offset inline and
  shares no code with `src/sta/Eapol.h`.

The captured transcript found two interop defects on its first run, both
inside the MIC and therefore invisible to any test where both ends are ours:
