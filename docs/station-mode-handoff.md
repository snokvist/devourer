...
20. **The 8822E station ACKs only ~83% of the AP's frames** once armed
    (8812EU station, 8812CU AP, ch36; ~17% station-side duplicates in three
    runs vs 0.01-0.05% for the 8822C/8822B stations). Ruled out: the peer's
    rate (MCS3 the same) and net_type (an uncommitted build arming AP type:
    the same). Candidates, untested: placement, and the combo chip's coex
    firmware taking response slots. The ACK-responder matrix measured this
    adapter at 98% single-shot, so it can do better. Retries hide it
    (downlink <= 0.22% loss).
