...
17. **Phase 6 remainder:** the 8821C cell (no adapter). The 8822E is done
    with a caveat - item 20. (Jaguar1 done:
...
20. ~~**The 8822E station ACKs only ~83% of the AP's frames**~~ **ROOT-CAUSED
    2026-09-26: the AP's receiver, near field.** An mt76 monitor witness (the
    MT7612U on its kernel driver, control frames on) saw the 8812EU's ACK on
    air after 96.8% of the airings the 8812CU AP re-aired - same 6M rate and
    signal as the ACKs it took - so the station sent them and the AP missed
    them. With only the station's power lowered 10 dB
    (`DEVOURER_STA_TXPWR_QDB=-40`, a new sta_client diagnostic): re-aired
    MSDUs 11% -> 0.6%, duplicates 669 -> 13. Roles swapped (8812CU station,
    8812EU AP): 0.2%. Placement on this bench, not the chip, not the arm.
    One run per arm; the analysis script self-calibrates the witness on the
    airings the AP accepted (ACK seen 99.9-100%).
