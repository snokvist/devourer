...
| `SetAckResponder` | ported, `ack_responder_ok = true` (normal ACK and BlockAck response) | singles: on 1725/1725 ACKed, retarget 1728/1728, and 0/1723 with retries pinned at 12 in the NEVER-ARMED cell (`run_phase` starts no responder when the responder MAC is empty, so that cell says nothing about a disarm — use the `disarmed` phase for that); A-MPDU armed/active-unarmed: 1.001/12.720 witnessed copies per payload and 14402/0 addressed BlockAck frames, with aggregation proven in both arms |
