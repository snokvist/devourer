...
own guidance they are worth about +/-3 points each — the HT column sitting
below the legacy column is not a finding on this evidence, and MCS0 reading
below MCS7 is a good illustration of why (MCS0 is the more robust rate, so the
ordering is noise, not a modulation cliff).
