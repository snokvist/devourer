...
| `SetAckResponder` | ported, `ack_responder_ok = true` (normal-ACK singles; BlockAck untested) | 8733B as responder: on 1725/1725 ACKed, retarget 1728/1728, off 0/1723 reports with retries pinned at 12 |
...
and counts every payload-counter event, avoiding the old sampled/quantized
`rx.txhit` readout. At retry 12 the witness saw 12.32–12.33 rather than the
ideal 13 copies/frame; passive-monitor loss (or genuinely fewer airings) keeps
that number a conservative observation rather than an exact retry count.
